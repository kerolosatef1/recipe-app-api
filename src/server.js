...
// ✅ POST: /api/favorites (بصيغة الجمع)
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(favoriteRecipes.recipeId, parseInt(recipeId))
        )
      );

    res
      .status(200)
      .json({ status: true, message: "Favorite recipe deleted successfully" });
  } catch (error) {
...
app.get("/api/favorites/:userId", async (req, res) => {
  try {
    ...
    if (!userId) {
      return res
        .status(400)
        .json({ status: false, message: "Missing required parameter" });
    }
